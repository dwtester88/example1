...
public class SenderReport {
	...
	private void send(long ntpts, long rtpts) throws IOException {
		...
		if (mTransport == TRANSPORT_UDP) {
			...
			usock.send(upack);
            /*Log.d("senderdata string: ",
                    upack.toString());
			Log.d("senderdata data: ",
					upack.getData().toString());
			Log.d("senderdata address: ",
					upack.getAddress().toString());
			Log.d("senderdata socketadd: ",
					upack.getSocketAddress().toString());*/
